...
import face_algorithm.MTCNN_keras.tools_matrix as tools
import cv2
import numpy as np
import time
    ...
    for rectangle in rectangles:
        if rectangle is not None:
            ...
            for i in range(5, 15, 2):
                ...
                print(int(rectangle[i + 0]), int(rectangle[i + 1]))
    #cv2.imshow("test", draw)
    #c = cv2.waitKey(1) & 0xFF
    # if c == 27 or c == ord('q'):
    #     break

    cv2.imwrite('../data/test.jpg', draw)
